  ...
  for (const sample of dataSample) {
    ...
    for (const sampleNum of sample.slice(0,3)) {
      for (const sampleNumIdx in sample.slice(0,3)) {
        ...
        if (!numShowSameTime[sampleNum][`${sampleNumIdx}-${sample[sampleNumIdx]}`]) {
          numShowSameTime[sampleNum][`${sampleNumIdx}-${sample[sampleNumIdx]}`] = 0
        }
        numShowSameTime[sampleNum][`${sampleNumIdx}-${sample[sampleNumIdx]}`] += 1
      }
    }
