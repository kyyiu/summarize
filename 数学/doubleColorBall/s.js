  ...
  try {
    ...
    https
      .get("https://zx.500.com/ssq/zhuanjiashahao.php?expectnum=30", (res) => {
        ...
        res.on("end", () => {
          write.write(`module.exports = ${JSON.stringify(nb1Numbers)}`);
        });
      })
      .on("error", (err) => {
...
const updateData = () => {
  try {
    ...
    https
      .get(
        "https://datachart.500.com/ssq/history/newinc/history.php?start=1&end=55047",
        (res) => {
          res.on("data", (chunk) => {
          ...
          res.on("end", () => {
            ...
            doc.replace(
              ...
              (match, num) => {
                ...
                if (curN.length === 7) {
                  write.write(
                    `"${curN.slice(0, 6).join("-")}#${curN.pop()}":1,\n`
                  );
                  curN = [];
                }
                return match; // 保持原字符串不变
