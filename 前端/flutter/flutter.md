...
itemExtent: 该参数如果不为null，则会强制children的“长度”为itemExtent的值;如果是垂直方向上滑动则是强制每个子元素的高度为itemExtent;使用这个参数会比直接使用子元素的尺寸有更好的性能，因为不必每次都去计算子元素后再获取到列表长度，而是可以直接获取到

什么时候用itemExtent,prototypeItem,shrinkWrap????
```         
...
当我们知道列表项的高度都相同时，强烈建议指定 itemExtent 或 prototypeItem 。下面看一个示例：
```dart
...
ScrollController({
  ...
  ,
  this.KeepScrollOffset = true, // 是否报错滚动位置
})
        ...
        child: Stack(
          ...
          children: <Widget>[
            ListView.builder(
              itemCount: 100,
              itemExtent: 50.0,
              itemBuilder: (context, index) => ListTitle(title: Text("$index")),
            ),
            CircleAvatar(
              //显示百分比
              radius: 30.0,
              child: Text(_progress),
              backgroundColor: Colors.blac54,
            )
          ],
        ),
      ),
...
示例，下面代码功能是： 手指在一个容器上移动时查看手指相对于容器的位置
```
